...
async function main() {
  ...
  console.log('Leyendo hojas del archivo ABUS...');
  const meta = await apiGet('/v4/spreadsheets/' + ABUS_ID + '?fields=sheets.properties', token);

  if (meta.error) {
    console.error('ERROR accediendo al archivo:', JSON.stringify(meta.error));
    console.log('\n⚠️  Asegurate de haber compartido el archivo con:', SA_EMAIL);
    return;
  }

  const sheets = meta.sheets || [];
  console.log('Hojas encontradas:', sheets.map(s => s.properties.title));

  // Leer primera hoja para ver la estructura
  const firstSheet = sheets[0].properties.title;
  console.log('\nLeyendo primeras 5 filas de "' + firstSheet + '"...');
  const r = await apiGet('/v4/spreadsheets/' + ABUS_ID + '/values/' + encodeURIComponent(firstSheet) + '!A1:H5', token);
  console.log('Estructura:');
  (r.values || []).forEach((row, i) => console.log('Fila ' + (i+1) + ':', JSON.stringify(row)));
}
