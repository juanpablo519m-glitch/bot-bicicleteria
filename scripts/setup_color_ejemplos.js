'use strict';
const https = require('https');
const crypto = require('crypto');

const SHEET_ID = process.env.SHEET_ID || '1qTMua-CQOeR3HrbcoCwoJKi9kW8foeEnzQhxIRKd3ps';
const SA_EMAIL = process.env.SA_EMAIL || 'bot-bicicleteria@n8n-bicicleteria.iam.gserviceaccount.com';
const SA_KEY   = (process.env.SA_PRIVATE_KEY || '').replace(/\\n/g, '\n') || `-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----`;

function getToken() {
  return new Promise((resolve, reject) => {
    const now = Math.floor(Date.now() / 1000);
    const hdr = Buffer.from(JSON.stringify({ alg: 'RS256', typ: 'JWT' })).toString('base64url');
    const cls = Buffer.from(JSON.stringify({ iss: SA_EMAIL, scope: 'https://www.googleapis.com/auth/spreadsheets', aud: 'https://oauth2.googleapis.com/token', exp: now + 3600, iat: now })).toString('base64url');
    const input = hdr + '.' + cls;
    const signer = crypto.createSign('RSA-SHA256');
    signer.update(input);
    const sig = signer.sign(SA_KEY).toString('base64url');
    const body = new URLSearchParams({ grant_type: 'urn:ietf:params:oauth:grant-type:jwt-bearer', assertion: input + '.' + sig }).toString();
    const req = https.request({ hostname: 'oauth2.googleapis.com', path: '/token', method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Content-Length': Buffer.byteLength(body) } }, res => {
      let d = ''; res.on('data', c => d += c);
      res.on('end', () => { try { resolve(JSON.parse(d).access_token); } catch { reject(d); } });
    });
    req.on('error', reject); req.write(body); req.end();
  });
}

function apiPut(path, data, token) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify(data);
    const req = https.request({ hostname: 'sheets.googleapis.com', path, method: 'PUT', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body), Authorization: 'Bearer ' + token } }, res => {
      let d = ''; res.on('data', c => d += c);
      res.on('end', () => { try { resolve(JSON.parse(d)); } catch { resolve(d); } });
    });
    req.on('error', reject); req.write(body); req.end();
  });
}

function apiPost(path, data, token) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify(data);
    const req = https.request({ hostname: 'sheets.googleapis.com', path, method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body), Authorization: 'Bearer ' + token } }, res => {
      let d = ''; res.on('data', c => d += c);
      res.on('end', () => { try { resolve(JSON.parse(d)); } catch { resolve(d); } });
    });
    req.on('error', reject); req.write(body); req.end();
  });
}

function apiGet(path, token) {
  return new Promise((resolve, reject) => {
    const req = https.request({ hostname: 'sheets.googleapis.com', path, method: 'GET', headers: { Authorization: 'Bearer ' + token } }, res => {
      let d = ''; res.on('data', c => d += c);
      res.on('end', () => { try { resolve(JSON.parse(d)); } catch { resolve(d); } });
    });
    req.on('error', reject); req.end();
  });
}

async function main() {
  const token = await getToken();
  console.log('✅ Token obtenido');

  // 1. Verificar headers actuales de STOCK
  const stockR = await apiGet(`/v4/spreadsheets/${SHEET_ID}/values/STOCK!A1:Z1`, token);
  const stockHeaders = stockR.values ? stockR.values[0] : [];
  console.log('Headers STOCK actuales:', stockHeaders.join(', '));

  // 2. Agregar columna 'color' a STOCK si no existe
  if (!stockHeaders.includes('color')) {
    const colS = String.fromCharCode(64 + stockHeaders.length + 1); // siguiente columna
    await apiPut(`/v4/spreadsheets/${SHEET_ID}/values/STOCK!${colS}1?valueInputOption=RAW`, { values: [['color']] }, token);
    console.log(`✅ Columna 'color' agregada en STOCK columna ${colS}`);
  } else {
    console.log('ℹ️  Columna color ya existe en STOCK');
  }

  // 3. Verificar headers actuales de FACTURAS
  const factR = await apiGet(`/v4/spreadsheets/${SHEET_ID}/values/FACTURAS!A1:Z1`, token);
  const factHeaders = factR.values ? factR.values[0] : [];
  console.log('Headers FACTURAS actuales:', factHeaders.join(', '));

  // 4. Agregar columna 'forma_pago' a FACTURAS si no existe
  if (!factHeaders.includes('forma_pago')) {
    const colF = String.fromCharCode(64 + factHeaders.length + 1);
    await apiPut(`/v4/spreadsheets/${SHEET_ID}/values/FACTURAS!${colF}1?valueInputOption=RAW`, { values: [['forma_pago']] }, token);
    console.log(`✅ Columna 'forma_pago' agregada en FACTURAS columna ${colF}`);
  } else {
    console.log('ℹ️  Columna forma_pago ya existe en FACTURAS');
  }

  // 5. Agregar 4 bicis de ejemplo (Raleigh Scout 2.0, mismo modelo, talles y colores distintos)
  const fecha = new Date().toLocaleDateString('es-AR');
  const ejemplos = [
    // tipo, marca, modelo, numero_serie, descripcion, ubicacion, stock_actual, stock_minimo, estado_unidad, precio_costo, precio_max, precio_min, rodado, talle, fecha_ingreso, ultima_actualizacion, ficha_tecnica, foto_url, color
    ['bicicleta','Raleigh','Scout 2.0','TEST-001','MTB aluminio doble disco','local','1','1','disponible','','150000','130000','26','S',fecha,fecha,'','','Verde'],
    ['bicicleta','Raleigh','Scout 2.0','TEST-002','MTB aluminio doble disco','local','1','1','disponible','','150000','130000','26','M',fecha,fecha,'','','Rojo'],
    ['bicicleta','Raleigh','Scout 2.0','TEST-003','MTB aluminio doble disco','local','1','1','disponible','','150000','130000','26','L',fecha,fecha,'','','Azul'],
    ['bicicleta','Raleigh','Scout 2.0','TEST-004','MTB aluminio doble disco','local','1','1','disponible','','150000','130000','26','XL',fecha,fecha,'','','Blanco'],
  ];

  await apiPost(
    `/v4/spreadsheets/${SHEET_ID}/values/STOCK!A1:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
    { values: ejemplos },
    token
  );
  console.log('✅ 4 bicis de ejemplo agregadas (Raleigh Scout 2.0 — S/M/L/XL)');
  console.log('\nListo! Reiniciá el bot y probá buscar "Raleigh" en el stock.');
}

main().catch(e => { console.error('ERROR:', e.message || e); process.exit(1); });
