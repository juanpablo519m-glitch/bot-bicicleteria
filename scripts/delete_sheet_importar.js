...
async function main() {
  ...
  console.log('Hojas existentes:', sheets.map(s => s.properties.title));

  const target = sheets.find(s => s.properties.title === 'IMPORTAR_PROVEEDOR');
  if (!target) {
    console.log('Hoja IMPORTAR_PROVEEDOR no encontrada. Nada que hacer.');
    return;
  }

  const sheetId = target.properties.sheetId;
  console.log('Eliminando IMPORTAR_PROVEEDOR (sheetId=' + sheetId + ')...');
  ...
  if (res.error) {
    ...
    console.log('✅ Hoja IMPORTAR_PROVEEDOR eliminada.');
  }
}
