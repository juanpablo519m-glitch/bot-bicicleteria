...
async function main() {
  ...
  const r = await apiGet('/v4/spreadsheets/' + SHEET_ID + '/values/STOCK!A1:P20', token);
  const rows = r.values || [];
  console.log('=== HEADERS ===');
  console.log(rows[0]);
  console.log('\n=== FILAS ===');
  rows.slice(1).forEach((row, i) => console.log(`Fila ${i+2}:`, JSON.stringify(row)));

  // Mostrar valores únicos de "tipo"
  const colTipo = rows[0].indexOf('tipo');
  const tipos = [...new Set(rows.slice(1).map(r => r[colTipo] || '(vacio)'))];
  console.log('\n=== TIPOS únicos ===', tipos);
}
