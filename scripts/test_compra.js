...
async function main() {
  ...
  // fecha, tipo, marca, modelo, descripcion, cantidad, precio_unitario, rodado, talle, ubicacion, estado, codigo_proveedor
  const fila = [fecha, 'bicicleta', 'stark', 'STRIVE 2.0 ADVANCE', 'DEORE 12V, Mono Promax', 1, 909090.91, '', '', 'local', '', '6170'];

  const body = JSON.stringify({ values: [fila] });
  const r = await new Promise((resolve, reject) => {
    const req = https.request({
      hostname: 'sheets.googleapis.com',
      path: '/v4/spreadsheets/' + SHEET_ID + '/values/COMPRAS!A:L:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS',
      method: 'POST',
  ...
  if (r.updates) console.log('✅ Fila de prueba agregada a COMPRAS. Ahora dale al botón → STOCK en el Excel.');
