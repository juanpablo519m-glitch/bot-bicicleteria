...
const { BOT_TOKEN, GROQ_KEY, ADMIN_ID, round5000 } = require('./config');
const { now, isEmpty, fuzzy, normalizarCampos } = require('./utils');
const { cache, refreshCache, appendRow, upsertRow, sortStock, syncVistaUbicacion, rebuildVistaBicis, calcularPrecios } = require('./sheets');
const { tgPost, tgSend, tgAnswer, mainMenu, rateLimit } = require('./telegram');
...
async function processUpdate(update) {
  ...
  const findProd  = q => {
    const buscaCuadro = /cuadro/.test(q.toLowerCase());
    return stock.filter(p => {
      if ((Number(p.stock_actual) || 0) <= 0) return false;
      if (['vendido','inactivo'].includes((p.estado_unidad || '').toLowerCase())) return false;
      const esCuadro = (p.tipo || '').toLowerCase() === 'cuadro';
      if (esCuadro && !buscaCuadro) return false;
      if (!esCuadro && buscaCuadro) return false;
      return fuzzy(q, p.numero_serie||'') || fuzzy(q, p.marca||'') || fuzzy(q, p.modelo||'') || fuzzy(q, p.tipo||'');
    });
  };
  ...
  const showProdDetail = async (p) => {
    ...
    kb.unshift([{ text: p.foto_url ? '📷 Cambiar foto' : '📷 Agregar foto', callback_data: `foto_${p.numero_serie}` }]);
  ...
  if (cb === 'movnew_ok' && estado === 'MOV_NUEVO_CONF') {
    ...
    const refProd = d.codigoProv ? cache.stock.find(s => s.codigo_proveedor && s.codigo_proveedor.toLowerCase() === d.codigoProv.toLowerCase() && (s.ficha_tecnica || s.foto_url)) : null;
    await appendRow('STOCK', { tipo: d.tipo, marca: d.marca, modelo: d.modelo, numero_serie: d.id, ubicacion: 'local', stock_actual: '0', stock_minimo: d.stMin, estado_unidad: 'disponible', precio_costo: d.precioCosto||'0', precio_max: d.precio, precio_min: d.precioMin||d.precio, rodado: d.rodado, talle: d.talle||'', fecha_ingreso: t, ultima_actualizacion: t, ficha_tecnica: refProd?.ficha_tecnica||'', foto_url: refProd?.foto_url||'', color: d.color||'', codigo_proveedor: d.codigoProv||'' });
  ...
  if (cb === 'transf_ok' && estado === 'MOV_TRANSF_CONF') {
    ...
    setTimeout(() => rebuildVistaBicis(), 2000);
    await clearSession();
  ...
  if (estado === 'MOV_PROD' && text) {
    ...
    if (res.length === 1) {
      const p = res[0];
      await saveSession('MOV_CANT', { ...datos, id_producto: p.numero_serie, numero_serie: p.numero_serie, marca: p.marca, modelo: p.modelo });
  ...
  if (cb === 'fact_ok' && estado === 'FACT_CONF') {
    const d = datos;
    await clearSession();
    await appendRow('FACTURAS', { id_factura: `FAC-${Date.now()}`, nombre: d.nombre, domicilio: d.domicilio, dni_cuit: d.dni_cuit, tipo: d.tipo, descripcion_producto: d.descripcion, precio_venta: d.precio, fecha: now(), forma_pago: '', numero_serie: '', factura_realizada: '', mail: d.mail||'', telefono: d.telefono||'' });
  ...
  if (cb === 'admin' && rol === 'administrador') {
    await tgSend(chatId, '⚙️ <b>Panel Admin:</b>',
      [[{ text: '👤 Agregar Usuario', callback_data: 'adm_user' }],
       ...
       [{ text: '📦 Agregar Stock', callback_data: 'fact_prov' }],
  ...
  if (estado === 'FACT_PROV_WAIT' && message && (message.photo || message.document)) {
    ...
    const [ocrRes, driveRes] = await Promise.allSettled([
      ...
      // uploadToDrive imported via sheets module
      require('./sheets').uploadToDrive(fileUrl, fileName, mimeType),
    ]);
  ...
  // ── Ver unidades de variante agrupada ─────────────────────────────────────
  ...
  if (cb === 'vr_ok' && estado === 'VR_CONF') {
    ...
    await clearSession();
    await appendRow(sheet, { fecha: now(), nombre, descripcion, precio, forma_pago, operador: user.nombre, factura_realizada: '' });
    await tgSend(chatId,
  ...
  if (cb.startsWith('venta_ok')) {
    ...
    const ventaTs = now();
    await appendRow('FACTURAS', { id_factura: `FAC-${Date.now()}`, nombre, domicilio, dni_cuit, tipo, descripcion_producto: descripcion, precio_venta: precio, fecha: ventaTs, factura_realizada: '', forma_pago, numero_serie, mail: mail||'', telefono: telefono||'' });
    ...
    await appendRow(hojaFac, { fecha: ventaTs, nombre, descripcion, precio, forma_pago, operador: user.nombre, factura_realizada: '' });
  ...
  if (cb.startsWith('ficha_')) {
    ...
    const p = stock.find(s => s.numero_serie === serie);
    if (!p) { await tgSend(chatId, 'Producto no encontrado.', [[{ text: '🏠 Menú', callback_data: 'main_menu' }]]); return; }
    const pmax  = Number(p.precio_max) > 0 ? '$'+round5000(Number(p.precio_max)).toLocaleString('es-AR', { maximumFractionDigits: 0 }) : 'n/n';
    const pmin  = Number(p.precio_min) > 0 ? '$'+round5000(Number(p.precio_min)).toLocaleString('es-AR', { maximumFractionDigits: 0 }) : 'n/n';
  ...
  if (estado === 'FOTO_WAIT' && message && (message.photo || message.document)) {
    ...
    if (tgUrl) {
      ...
      const { uploadToDrive } = require('./sheets');
  ...
  // ── Venta rápida desde ficha de producto ──────────────────────────────────
  ...
  if (cb.startsWith('vrap_ok')) {
    ...
    await appendRow(hoja, { fecha: ventaTs, nombre, descripcion, precio: precio || '', forma_pago, operador: user.nombre, factura_realizada: '' });
    await upsertRow('STOCK', { numero_serie, stock_actual: String(newStock), estado_unidad: newStock === 0 ? 'vendido' : (p.estado_unidad || 'disponible'), ultima_actualizacion: ventaTs }, 'numero_serie');
    setTimeout(() => rebuildVistaBicis(), 2000);
