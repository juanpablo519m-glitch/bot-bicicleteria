...
async function rebuildVistaBicis() {
  try {
    ...
    for (const marca of marcas) {
      ...
      const displayNombre = marcaDisplay[marca];
      titleRowIdxs.push(allRows.length);
      allRows.push([`🚲  ${displayNombre.toUpperCase()}  —  ${items.length} unidad(es)`]);
      headerRowIdxs.push(allRows.length);
