...
async function sortStock() {
  try {
    ...
    // Ordenar: marca con más items primero, luego por marca, modelo DESC, precio DESC
    rows.sort((a, b) => {
    ...
    await axios.put(
      `${SHEETS_BASE}/${SHEET_ID}/values/STOCK!A2:${endCol}${filled.length + 1}?valueInputOption=RAW`,
      { values: filled },
...
module.exports = {
  ...
  sortStock, syncVistaUbicacion, calcularPrecios,
};
